...
class Controller extends \Controller
{
    ...
    public function post($f3): void
    {
        if ($f3->get("GET.token") == $f3->get(\Plugin\Bitbucket\Base::CONFIG_KEY_TOKEN)) {
            ...
            if (str_starts_with($post, 'payload=')) {
                parse_str($post, $data);
                $post = $data['payload'];
            }
            ...
            foreach ($json->push->changes as $change) {
                ...
                foreach ($change->commits as $commit) {
                    ...
                    if (preg_match("/#\\d+/", (string) $commit->message, $matches)) {
                        ...
                        if ($issue->id) {
                            ...
                            // Find matching user
                            if (preg_match("/<[^ ]+@[^ ]+>/", (string) $commit->author->raw, $matches)) {
                            ...
                            if (!$issue->closed_date && preg_match("/#(resolve|fix|close)/i", (string) $commit->message)) {
                            ...
                            elseif ($issue->closed_date && preg_match("/#(re-?open|new|broken)/i", (string) $commit->message)) {
                                $status = new \Model\Issue\Status();
                            ...
                            if (preg_match("/@[0-9\.]+h/i", (string) $commit->message, $matches)) {
                                $hours = floatval(trim($matches[0], "@Hh"));
                                if ($hours !== 0.0) {
                                    $issue->hours_spent += $hours;
                                    $updated = true;
                                }
                            }
